package views 
{
	...
	public class Menu extends DView 
	{
		...
		override public function init():void {
			...
			skin.btnInst.buttonMode = true;
			skin.btnPlay.alpha = 0;
			skin.title.alpha = 0;
			skin.btnInst.alpha = 0;
			...
			skin.btnInst.addEventListener(MouseEvent.CLICK, goInst);
		}
		
		private function goInst(e:MouseEvent):void 
		{
			timeline.reverse();
			timeline.addEventListener(TweenEvent.REVERSE_COMPLETE, reverseInst);
		}
		
		private function reverseInst(e:TweenEvent):void 
		{
			dispatchEvent(new NavigationEvent(NavigationEvent.CHANGE_VIEW,'view_inst'));
		}
		override public function initAnim():void 
		{
			timeline = new TimelineMax();
			timeline.append(TweenLite.to(skin.bg, 0.5, { y:240,alpha:1 } ));
			timeline.append(TweenLite.to(skin.title, 0.5, { alpha:1 } ));
			timeline.append(TweenLite.to(skin.btnPlay, 0.5, { alpha:1 } ));
			timeline.append(TweenLite.to(skin.btnInst, 0.5, { alpha:1 } ));
